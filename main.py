...
parser.add_argument('--pretrain', dest='pretrain', action='store_true',
                    help='pretrain model on other data set')
parser.add_argument('--finetune', dest='finetune', action='store_true',
                    help='finetune model from pretrained model on other data set')
parser.add_argument('--dir', default='pth', type=str, metavar='model path',
                    help='dir to save models (default: pth)')
# parser.add_argument('--gpus', default=None, nargs='+', type=int,
#                     help='multiple GPU ids to use.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.vit:      
        ...
        model = ViTLSTM_nofc(args.arch, pretrained=args.pretrained, num_classes=num_classes, image_size=args.image_size)
    ...
    if args.distributed:
        ...
        device = list(range(ngpus_per_node))
    ...
    # valdir = os.path.join(args.data,'sub'+str(args.sub_val).zfill(2))
    ...
    #     traindir,
    #     transforms.Compose([
    #         transforms.RandomResizedCrop(args.image_size), #delete
    #         transforms.RandomHorizontalFlip(), #delete
    #         transforms.ToTensor(),
    #         normalize,
    #     ]))
    
    # from self defined datasets.py
    ...
    if args.pretrain:
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # train for one epoch
            acc1 = train(train_loader, model, criterion, optimizer, scheduler, epoch, args)
...
def train(train_loader, model, criterion, optimizer, scheduler, epoch, args):
    ...
    for i, (images, targets) in enumerate(train_loader, 1):
        ...
        #     outputs.append(output/args.num_images)
...
def validate(val_loader, model, criterion, args, loso=False):
    ...
    with torch.no_grad():
        ...
        for i, (images, targets) in enumerate(val_loader, 1):
            # outputs = []
            # # TODO 03.08.2021: combine the validate method in previous MTL code
            # for (image, target) in zip(images, targets):
            #     # if args.gpu is not None:
            #     #     image = image.cuda(args.gpu, non_blocking=True)
            #     #     target = target.cuda(args.gpu, non_blocking=True)
