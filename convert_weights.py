...
npz_files = {
    # 'B_16': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-B_16.npz',
    # 'B_32': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-B_32.npz',
    # # 'L_16': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-L_16.npz',  # <-- not available
    # 'L_32': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-L_32.npz',
    'B_16_imagenet1k': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-B_16_imagenet1k.npz',
    # 'B_32_imagenet1k': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-B_32_imagenet1k.npz',
    # 'L_16_imagenet1k': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-L_16_imagenet1k.npz',
    'L_32_imagenet1k': 'PyTorch_ViT/jax_to_pytorch/jax_weights/ViT-L_32_imagenet1k.npz',
}
...
def check_model(model, name):
    model.eval()
    img = Image.open('PyTorch_ViT/examples/simple/img.jpg')
    ...
    if 'imagenet1k' in name:
        labels_file = 'PyTorch_ViT/examples/simple/labels_map.txt' 
        labels_map = json.load(open(labels_file))
        labels_map = [labels_map[str(i)] for i in range(1000)]
        print('-----\nShould be index 388 (panda) w/ high probability:')
    else:
        print('~ not checked ~')
        return # labels_map = open('PyTorch_ViT/examples/simple/labels_map_21k.txt').read().splitlines()
    with torch.no_grad():
...
for name, filename in npz_files.items():
    ...
    # Save weights
    new_filename = f'PyTorch_ViT/jax_to_pytorch/weights/{name}.pth'
